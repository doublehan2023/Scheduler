import { CalendarDays, LayoutDashboard, LogOut, Users, Wand2 } from "lucide-react";
...
const SideBar = ({
  ...
  const navItems = [
    ...
    { name: "Scheduler", icon: CalendarDays, path: "/scheduler" },
  ...
  return (
    <div
      className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 flex flex-col
        h-full transform transition-transform duration-200 ease-in-out md:relative md:translate-x-0 
    ...
    >
          ...
          return (
            <NavLink
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded text-sm
                transition-all duration-150 border ${isActive ? "bg-red-50 text-red-600 border-red-100" : "text-slate-500 hover:bg-slate-50 border-transparent hover:text-slate-700"}`}
            >
              <item.icon
      ...
      <div className="p-4 border-t border-slate-100">
        <div className="flex items-center gap-3 p-2 rounde-xl hover:bg-slat-50 transition-colors">
